...
while ($file =~ /((?:\w| )+)<\/td><td>(\w+)<\/td><td>(\w+|-)<\/td><td>(\w+|-)(<\/td>)<td>(\w+)\D+(\d+)\D+(\d+)\D+(\d+)\D+?(\d+|-)\D+(\d+)\D+(\d+)\D+(\d+)\D+(\d+)\D+\d+\D+\d+.+?(\w+)<\/td>/g){
	my $dmg = $10;
	$dmg = 0 if $dmg eq '-';
	my $class = $classes{$2};
	$st->execute($1,$class,$3,$4,'-',$6,$7,$8,$9,$dmg,$11,$12,$13,$14,$15) or die $dbh->errstr;
	print "$1,$class,$3,$4,'-',$6,$7,$dmg,$9,$10,$11,$12,$13,$14,$15\n";
}
